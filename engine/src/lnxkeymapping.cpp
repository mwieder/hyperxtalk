...
{
  int min = 0;
  int max = NUM_ELEMENTS (keysym_to_unicode_tab) - 1;
  int mid;
  ...
  while (max >= min) {
    mid = (min + max) / 2;
    if (keysym_to_unicode_tab[mid].keysym < keyval)
      min = mid + 1;
    else if (keysym_to_unicode_tab[mid].keysym > keyval)
      max = mid - 1;
    else {
      /* found it */
...
{
	// Any codepoint < 65536 is encoded as a single 16-bit
	// value.
	if (p_codepoint <= 0xFFFF)
	{
		*p_buffer = p_codepoint;
		return 1;
	}

	// Any codepoint >= 65536 needs to be split into two 16-bit
	// values called surrogates.

	// First remove the high-bit from the codepoint and split
	// into to the high and low 10 bits.

	unsigned int t_high;
	t_high = (p_codepoint - 0x10000) >> 10;

	unsigned int t_low;
	t_low = (p_codepoint - 0x10000) & 1023;

	// Now recombine with the special surrogate bits:
	//   0xD800 for the 1st surrogate
	//   0xDC00 for the 2nd surrogate

	p_buffer[0] = 0xD800 | t_high;
	p_buffer[1] = 0xDC00 | t_low;
